...
let intro_name s fmt l =
  if l = [] then () else
  fprintf fmt "Variables %a :@ %s.@ " (list space Name.print) l s
...
let pr_generalize in_term kind fmt ((tl,rl,el) as g) =
  if Ty.Generalize.is_empty g then ()
    ...
    | `Pangoline ->
        fprintf fmt "forall %t %a." in_term (list space Name.print) tl
...
and theory kind fmt l = list newline (print kind) fmt l
