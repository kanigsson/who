...
Section sec. 
  ...
  Variable l:  list a. 
  Lemma list_length_pos: ((@List_length  a) l) >= 0. 
    Proof.
    elim l.
    assert (List_length (@nil a) = 0).
    admit. (* previous proof *)
    
    omega.
    
    intros.
    assert (List_length (Cons a0 l0) = List_length l0 + 1).
      pose proof (list_length_ind (Cons a0 l0) (List_length l0 + 1)) as H0. 
      destruct H0.
      symmetry. apply H0.
      assert (ind_list_len l0 (List_length l0)).
        pose proof (list_length_ind l0 (List_length l0)). destruct H2.
        apply H3.
        trivial.
