...
Section sec. 
  ...
  Variable l:  list a. 
  Lemma g:
    ((@List_length  a) ((@Cons  a) x ((@Cons  a) x1 l))) >= 2. 
    Proof.
    remember (Cons x (Cons x1 l)) as old.
    assert (List_length old = List_length l  +2 ).
        assert (ind_list_len l (List_length l)).
          pose proof (list_length_ind l (List_length l)).
          destruct H.
          apply H0. trivial.
        
        assert (ind_list_len (Cons x1 l) (List_length l+1)).
          apply (recur _ H).
        assert (ind_list_len (Cons x1 l) (List_length (Cons x1 l))).
          pose proof (list_length_ind (Cons x1 l) (List_length (Cons x1 l))).
          destruct H1.
          apply H2. trivial.
     
      assert (ind_list_len old (List_length (Cons x1 l)+1)).
        rewrite Heqold. 
        
         
      apply (recur x H1).
      
      assert (List_length l +2 = (List_length l+1) +1).
        omega.

      rewrite H3.
      assert (ind_list_len old ((List_length l+1) +1)).
       rewrite Heqold.
       apply (recur x H0).

      pose proof (list_length_ind (Cons x (Cons x1 l)) ((List_length l+1) +1)).
      destruct H5. rewrite Heqold.
      symmetry. apply H5.
      admit.
      
      pose proof (list_length_ind old (List_length l+2)). 
      destruct H0.
      assert (List_length l >= 0).
        apply list_length_pos.

      omega.
      Qed. (* this proof is far too long *)
